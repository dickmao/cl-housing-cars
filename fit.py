...
def main():
    ...
    argparser.add_argument('--corenlp-host', type=str, default='http://localhost:9005')
    ...
    client = CoreNLPClient(start_cmd="gradle -p {} server".format(os.getenv("CORENLP_HOME")), endpoint=args.corenlp_uri, timeout=15000)
    cr = Json100CorpusReader(wdir, jsons, dedupe="id", word_tokenizer=WordTokenizer(client), sent_tokenizer=SentTokenizer(client))
    crabo = Json100CorpusReader(wdir, jsons, dedupe="id", link_select='abo', word_tokenizer=WordTokenizer(client), sent_tokenizer=SentTokenizer(client))
    crsub = Json100CorpusReader(wdir, jsons, dedupe="id", link_select='sub', word_tokenizer=WordTokenizer(client), sent_tokenizer=SentTokenizer(client))
    crjoin = Json100CorpusReader(wdir, ["joinery.json"], dedupe="id", word_tokenizer=WordTokenizer(client), sent_tokenizer=SentTokenizer(client))
    crlp = Json100CorpusReader(wdir, ["listingsproject.json"], dedupe="id", word_tokenizer=WordTokenizer(client), sent_tokenizer=SentTokenizer(client))
    ...
    svc.score(crtest_desc, crtest_labels)
