...
class CraigSpider(BaseSpider):
    ...
    def __init__(self, name=None, **kwargs):
        super(CraigSpider, self).__init__(name, **kwargs)
        self.states = dict()

    def _set_crawler(self, crawler):
        super(CraigSpider, self)._set_crawler(crawler)
        if self.Marker:
            marker = self.download_s3(self.Marker, deserialize=json.loads)
